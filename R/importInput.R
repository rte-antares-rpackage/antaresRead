...
.importInputTS <- function(area, timeStep, opts, fileNamePattern, colnames,
  ...
  if(!is.null(inputTS)){
    # Add area and timeId columns and put them as first columns of the table
    inputTS$area <- area
...
.importminReservoirLevels <- function(area, timeStep, opts, ...){

  if (opts[["antaresVersion"]] < 101000) {
    stop("minReservoirLevels time series is available since Antares 10.1", call. = FALSE)
  }

  .importInputTS(area, timeStep, opts, "hydro/series/%s/minDailyReservoirLevels.txt", "minReservoirLevels",
                 inputTimeStep = "daily", type = "matrix")
}


.importavgReservoirLevels <- function(area, timeStep, opts, ...){

  if (opts[["antaresVersion"]] < 101000) {
    stop("avgReservoirLevels time series is available since Antares 10.1", call. = FALSE)
  }

  .importInputTS(area, timeStep, opts, "hydro/series/%s/avgDailyReservoirLevels.txt", "avgReservoirLevels",
                 inputTimeStep = "daily", type = "matrix")
}


.importmaxReservoirLevels <- function(area, timeStep, opts, ...){

  if (opts[["antaresVersion"]] < 101000) {
    stop("maxReservoirLevels time series is available since Antares 10.1", call. = FALSE)
  }

  .importInputTS(area, timeStep, opts, "hydro/series/%s/maxDailyReservoirLevels.txt", "maxReservoirLevels",
                 inputTimeStep = "daily", type = "matrix")
}
